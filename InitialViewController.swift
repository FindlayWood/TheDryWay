...
class InitialViewController: UIViewController, GMSMapViewDelegate ,  CLLocationManagerDelegate {
    ...
    //value for holding the temperature
    ...
    //points holds all coordinates along route, metres holds duration between those points
    ...
    var infoRegisterRefferenceVC : MapViewController?
    
    func saveContrainerViewRefference(vc:MapViewController){
    ...
    @IBAction func driveButton(_ sender: UIButton) {
        ...
        Flurry.logEvent("Drive Button")
        
    }
    
    func clear(_ mapView: GMSMapView){
        infoRegisterRefferenceVC?.mapView?.clear()
        infoRegisterRefferenceVC?.heldLocations.removeAll()
        infoRegisterRefferenceVC?.startTaken = false
        //self.mapView?.clear()
    }
    ...
    func removingFromArrays(){
        ...
        self.currentTemperatures.removeAll()
    }
...
//                    }
    ...
    @IBAction func openStartLocation(_ sender: UIButton) {
        ...
        // selected location
        infoRegisterRefferenceVC?.locationSelected = .startLocation
        
        // Change text color
        UISearchBar.appearance().setTextColor(color: UIColor.black)
        //self.locationManager.stopUpdatingLocation()
        infoRegisterRefferenceVC?.locationManager.stopUpdatingLocation()
    ...
    @IBAction func openDestinationLocation(_ sender: UIButton) {
        ...
        // selected location
        infoRegisterRefferenceVC?.locationSelected = .destinationLocation
...
//    }
    ...
    @IBAction func showDirection(_ sender: UIButton) {
        ...
        removingFromArrays()
        clear((infoRegisterRefferenceVC?.mapView)!)
        self.bicyclingButton.isSelected = true
        self.transportMethod = "bicycling"
        self.routes = 0
                ...
                do {
                    if let json = try JSONSerialization.jsonObject(with: data, options: []) as? [String:Any] {
                        ...
                        if let currentForecasts = json["currently"] as? [String:Any]{
                            if let temp = currentForecasts["temperature"] as? Double{
                                self.currentTemperatures.append(temp)
                                //let x = ((temp - 32) * (5.0/9.0))
                                //self.temperatures = String(Int(x))
                            }
                        }
                    }
                }catch {
...
extension InitialViewController: GMSAutocompleteViewControllerDelegate {
    ...
    func viewController(_ viewController: GMSAutocompleteViewController, didAutocompleteWith place: GMSPlace) {
        ...
        if infoRegisterRefferenceVC?.locationSelected == .startLocation {
            startLocation.text = "\(place.name)"
            locationStart = CLLocation(latitude: place.coordinate.latitude, longitude: place.coordinate.longitude)
            infoRegisterRefferenceVC?.createMarker(titleMarker: "Location Start", iconMarker: #imageLiteral(resourceName: "pin") , latitude: place.coordinate.latitude, longitude: place.coordinate.longitude)
        } else {
            destinationLocation.text = "\(place.name)"
            locationEnd = CLLocation(latitude: place.coordinate.latitude, longitude: place.coordinate.longitude)
            infoRegisterRefferenceVC?.createMarker(titleMarker: "Location End", iconMarker: #imageLiteral(resourceName: "pin"), latitude: place.coordinate.latitude, longitude: place.coordinate.longitude)
        }
