...
//  Mapping
//
//  Created by Findlay Wood on 08/09/2018.
...
class MapViewController: UIViewController, GMSMapViewDelegate ,  CLLocationManagerDelegate {
    ...
    var heldLocations = [CLLocationCoordinate2D]()
...
//
    ...
    func didTapMyLocationButton(for mapView: GMSMapView) -> Bool {
        ...
        Flurry.logEvent("My Location tapped")
        return false
    }
    
    func mapView(_ mapView: GMSMapView, didLongPressAt coordinate: CLLocationCoordinate2D) {
        let marker = GMSMarker(position: coordinate)
        if heldLocations.count == 0 && startTaken == false{
            heldLocations.append(coordinate)
            marker.title = "Start Point"
            marker.map = mapView
            delegate.startLocation.text = "\(coordinate.latitude), \(coordinate.longitude)"
            delegate.locationStart = CLLocation(latitude: coordinate.latitude, longitude: coordinate.longitude)
        }else{
    ...
    override func viewDidAppear(_ animated: Bool) {
        ...
        let signUpControllerParent = self.parent as! InitialViewController
